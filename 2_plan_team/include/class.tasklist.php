...
class tasklist extends TableBase
{
    ...
    */
    ...
    function getProjectTasklists($project, $status = 1)
    {
        ...
        $sel = mysql_query("SELECT * FROM ".$this->getTableName()." WHERE project = $project".$sql_status." ORDER BY startdate ASC, name ASC");
        $tasklists = array();
    ...
    function getTasklist($id)
    {
        ...
        if (!empty($tasklist))
        {
            ...
            $startdate = date(CL_DATEFORMAT, $tasklist["startdate"]);
            $finishdate = date(CL_DATEFORMAT, $tasklist["finishdate"]);
